...
### Exit code 優先順位（決定表）
> 「どれか1つでもローカル保存が壊れたら非0」「Telegram はベストエフォート」を機械的に運用できるよう固定する。

- ルール:
  1) 個別ログ（raw）の保存に失敗 → **exit non-zero**（最優先）
  2) summary 再生成に失敗 → **exit non-zero**
  3) 1)2) が成功している場合:
     - `--telegram` 無し → **exit 0**
     - `--telegram` あり:
       - env 不足 / payload 必須項目欠損 → warn + **exit 0**（送信しない）
       - Telegram API 失敗（429/network/400/403 等）→ warn + **exit 0**
- 補足:
  - permissions（chmod等）は best-effort とし、内容の保存が成功していれば warn + 継続（exit code には影響させない）。
  - ロック取得や atomic replace の失敗は「保存の失敗」として扱う（exit non-zero）。
...
- 関連仕様（requirement/design/plan/report）:
  - `spec-dock/initiatives/init-00001-codex-notify-json-logger/adrs/adr-00001-notify-logger-output-and-telegram.md`
