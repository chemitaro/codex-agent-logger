...
- 決定: ファイル名に埋め込む safe id は **複合ID（thread-id + turn-id）の短縮ハッシュのみ**（Option B）を採用する。
  - `event-key = f"{thread-id}:{turn-id}"`
    - 欠損/不正時はセンチネル文字列で補完してから生成（実装の揺れを防ぐため固定する）:
      - `thread-id`: `"<missing-thread-id>"`
      - `turn-id`: `"<missing-turn-id>"`
    - payload JSON が parse できない場合も、`thread-id`/`turn-id` は上記センチネルとして扱う
...
- 前提:
  ...
  - 同時実行による衝突回避のため、ログファイルは「存在確認→作成」ではなく **排他的・原子的作成（`O_EXCL` 相当）**で行い、失敗時のみ suffix を増やして再試行する。
...
- 影響範囲（コード/テスト/運用/データ）:
  - `epic-00003` のログファイル命名とテスト
- 移行/ロールバック:
  - safe id 方式を変更するとファイル名パターンが変わるが、summary は `logs/*.json` を走査して再生成するため互換は保ちやすい
- Follow-ups（追加の Epic/Issue/ADR）:
  - 決定後、`accepted` に更新し、`epic-00003` の TBD を解消する

## 参考（References） (任意)
- 関連仕様（requirement/design/plan/report）:
  - `spec-dock/initiatives/init-00001-codex-notify-json-logger/epics/epic-00003-local-logging-and-summary/requirement.md`
  - `spec-dock/initiatives/init-00001-codex-notify-json-logger/epics/epic-00003-local-logging-and-summary/plan.md`
- PR/実装:
  - （未実装）
- 外部資料:
  - N/A
