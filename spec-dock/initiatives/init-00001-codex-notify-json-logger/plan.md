...
# init-00001 Codex Notify Json Logger — 計画（Roadmap / Epics）
...
## 順序と理由（Sequencing） (必須)
- なぜこの順番か（依存/リスク/価値提供）:
  - まず uvx 実行できる CLI/配布（epic-00002）を整備し、最短で「実行→テスト→改善」のループを作る
  - ローカル保存が SSOT（必達）なので次に epic-00003 を実装する
  - Telegram は任意（外部依存/権限/漏洩リスク）があるため最後に epic-00004 を実装する
- 並行できるもの / できないもの:
