...
- 影響範囲（コード/テスト/運用/データ）:
  - `epic-local-00002` の分割送信ロジック
  - テスト（連番付与 + 4096 超過しないこと）
- 移行/ロールバック:
  - 連番は UI 表示のみで、mapping やローカル保存には影響しない
- Follow-ups（追加の Epic/Issue/ADR）:
  - 決定後、この ADR を `accepted` にし、`epic-local-00002` の実装/テストへ反映する

## 参考（References） (任意)
- 関連仕様（requirement/design/plan/report）:
  - `spec-dock/initiatives/init-local-00001-codex-notify-json-logger/epics/epic-local-00002-telegram-topics-delivery/requirement.md`
- PR/実装:
  - （未実装）
- 外部資料:
  - Telegram 4096 文字制限
